...
PY
else API=https://api.github.com/repos/XTLS/Xray-core/releases; JSON="$TMP_DIR/releases.json"; log "Fetching GitHub releases list..."; curl -fsSL "$API" -o "$JSON"; python3 - "$ASSET" "$JSON" <<'PY'
import sys,json
asset,path=sys.argv[1],sys.argv[2]
...
#!/bin/sh
BACKUP_DIR="/opt/backup/xray-core"; TARGET="/opt/sbin/xray"; LIST="/opt/tmp/xray-core-rollback-list.txt"; : > "$LIST"; i=1
...
printf "Restore $SELECTED ? 1=yes: "; read c; [ "$c" = 1 ] || exit 0
/opt/etc/init.d/S27xray-watchdog stop >/dev/null 2>&1 || true; /opt/etc/init.d/S28xray-proxy0-watchdog stop >/dev/null 2>&1 || true; /opt/etc/init.d/S25xray-failover stop >/dev/null 2>&1 || true; /opt/etc/init.d/S24xray stop >/dev/null 2>&1 || true
cp "$SELECTED" "$TARGET"; chmod +x "$TARGET"; "$TARGET" version
/opt/etc/init.d/S24xray restart; /opt/etc/init.d/S25xray-failover restart; /opt/etc/init.d/S27xray-watchdog restart; /opt/etc/init.d/S28xray-proxy0-watchdog restart
EOF
...
while true; do header; echo "Главное меню"; echo "--------------------------------------"; echo "1) Статус / диагностика"; echo "2) Failover Primary / Backup"; echo "3) Xray_main / Proxy1"; echo "4) Watchdog-и"; echo "5) Обновление Xray core"; echo "6) Backup / Restore / Rollback"; echo "7) Логи"; echo "8) Proxy repair"; echo "0) Выход"; echo; printf "Введите номер: "; read c; case "$c" in 1) status_menu;; 2) failover_menu;; 3) xray_main_menu;; 4) watchdog_menu;; 5) core_update_menu;; 6) backup_menu;; 7) logs_menu;; 8) proxy_menu;; 0) echo "Выход."; exit 0;; *) echo "Неверный пункт."; pause;; esac; done
EOF
...
echo "[1/12] Stop old services"
stop_old_services

echo "[2/12] Ensure packages"
ensure_packages

echo "[3/12] Prepare router IP and links"
prepare_links

echo "[4/12] Create generator"
create_generator

echo "[5/12] Create init scripts"
create_init_scripts

echo "[6/12] Create proxy repair commands"
create_proxy_repair_commands

echo "[7/12] Create failover daemon"
create_failover_daemon

echo "[8/12] Create watchdogs"
create_watchdogs

echo "[9/12] Create Xray_main commands"
create_xray_main_commands

echo "[10/12] Create update/switch/status commands"
create_update_switch_status_commands
create_core_update_commands
create_status_log_maintenance_commands
create_maintenance_tools
