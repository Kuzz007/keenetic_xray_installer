...
# Minimal Go repair backend shim.
# The full low-storage Minimal Go installer was removed from main, but deployed
# routers still need its repair-only path to refresh minimal-go-status/switch/update.
# Keep this top-level path stable and fetch the last known-good backend by commit.
...
OUT="$TMP_DIR/xray_vless_failover_minimal_old_go.backend.$$"
...
echo "Downloading Minimal Go repair backend..."
