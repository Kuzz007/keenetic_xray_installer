...
# by forwarding to the canonical Go/Entware installer.

REPO_BASE="${REPO_BASE:-https://raw.githubusercontent.com/Kuzz007/keenetic_xray_installer/main}"
TARGET_URL="${GO_FULL_CANONICAL_URL:-$REPO_BASE/xray_vless_failover_go.sh}"
TMP_DIR="${TMP_DIR:-/opt/tmp}"
[ -d "$TMP_DIR" ] || mkdir -p "$TMP_DIR"
TMP="$TMP_DIR/xray_vless_failover_go.sh.$$"
...
if ! curl -fsSL -o "$TMP" "$TARGET_URL"; then
    echo "ERROR: failed to download canonical Go installer: $TARGET_URL" >&2
    exit 1
fi
