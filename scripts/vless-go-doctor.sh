...
check_xray_init_status() {
    if [ ! -x "$XRAY_INIT" ]; then
        warn "Xray init script not found: $XRAY_INIT; creating it"
        if create_xray_init; then ok "Xray init script created: $XRAY_INIT"; else fail "failed to create Xray init script"; return 0; fi
    fi
    if "$XRAY_INIT" status >/tmp/vless-go-doctor.status.$$ 2>&1; then
        ok "Xray init status: alive"
    else
        warn "Xray init status is not alive; attempting start"
...
check_storage() {
    ...
    else warn "failed to read /opt disk usage"; fi
...
check_go_resolver() { if ! has_cmd xray-failover-go; then fail "xray-failover-go not found"; return 0; fi; VERSION="$(xray-failover-go -version 2>/dev/null || true)"; if [ -n "$VERSION" ]; then ok "xray-failover-go version: $VERSION"; else warn "xray-failover-go -version failed; binary may be old"; fi; HELP="$(xray-failover-go -h 2>&1 || true)"; for FLAG in -select-index -select-name -json -private -version; do if echo "$HELP" | grep -q -- "$FLAG"; then ok "xray-failover-go supports $FLAG"; else fail "xray-failover-go missing $FLAG"; fi; done; }
...
section "Commands"; for CMD in opkg curl xray xray-failover-go vless-go-update vless-go-failover vless-go-watchdog vless-go-auto-update vless-go-xray-core-update vless-go-history vless-go-cleanup xray-go-installer-update failover-go; do if has_cmd "$CMD"; then ok "$CMD found: $(command -v "$CMD")"; else fail "$CMD not found"; fi; done
section "Storage"; check_storage
section "Architecture"; check_architecture
section "Go resolver"; check_go_resolver
section "Updater dependencies"; for CMD in python3 unzip; do if has_cmd "$CMD"; then ok "$CMD found: $(command -v "$CMD")"; else warn "$CMD not found; vless-go-xray-core-update can install it via opkg when needed"; fi; done; if has_cmd vless-go-xray-core-update; then if vless-go-xray-core-update --help >/tmp/vless-go-doctor.corehelp.$$ 2>&1; then ok "Xray-core updater help works"; else warn "Xray-core updater help returned non-zero"; sed 's/^/  /' /tmp/vless-go-doctor.corehelp.$$; fi; rm -f /tmp/vless-go-doctor.corehelp.$$ 2>/dev/null || true; fi
section "Saved state"; check_file_present "$SOURCE_STORE" "current source"; check_file_present "$PRIMARY_STORE" "primary source"; check_file_present "$BACKUP_STORE" "backup source"; ACTIVE="$(read_first "$ACTIVE_STORE")"; case "$ACTIVE" in primary|backup) ok "active slot: $ACTIVE" ;; '') warn "active slot is not set" ;; *) fail "active slot invalid: $ACTIVE" ;; esac; check_selector "$PRIMARY_SELECTOR" "primary"; check_selector "$BACKUP_SELECTOR" "backup"; [ -s "$WATCHDOG_CONF" ] && ok "watchdog config present: $WATCHDOG_CONF" || warn "watchdog config missing: $WATCHDOG_CONF"
section "Xray"; XRAY_BIN="$(xray_bin)"; if [ -n "$XRAY_BIN" ]; then ok "xray binary: $XRAY_BIN"; XRAY_VERSION="$($XRAY_BIN version 2>/dev/null | sed -n '1p')"; [ -n "$XRAY_VERSION" ] && ok "Xray version: $XRAY_VERSION" || warn "failed to read Xray version"; if [ -s "$XRAY_CONFIG" ]; then ok "Xray config present: $XRAY_CONFIG"; if "$XRAY_BIN" run -test -config "$XRAY_CONFIG" >/tmp/vless-go-doctor.xray.$$ 2>&1; then ok "Xray config validation OK"; else fail "Xray config validation failed"; sed 's/^/  /' /tmp/vless-go-doctor.xray.$$; fi; rm -f /tmp/vless-go-doctor.xray.$$ 2>/dev/null || true; else fail "Xray config missing: $XRAY_CONFIG"; fi; else fail "xray binary not found"; fi; check_backups; check_xray_init_status; check_socks_listener; check_socks_health
section "Proxy0"; check_proxy0
section "Auto-update"; if has_cmd vless-go-auto-update; then vless-go-auto-update status 2>/dev/null | sed 's/^/  /'; fi; check_cron_auto_update; [ -s "$AUTO_UPDATE_LOG" ] && show_tail_safe "$AUTO_UPDATE_LOG" "auto-update log" || info "auto-update log empty or missing: $AUTO_UPDATE_LOG"
section "Watchdog"; check_init_status "$WATCHDOG_INIT" "VLESS Go watchdog"; show_watchdog_summary
section "Switch history"; check_history
section "Logs"; show_tail_safe "$WATCHDOG_LOG" "watchdog main log"; if [ "$VERBOSE" = "1" ]; then echo "-- watchdog detail log --"; echo "  WARNING: verbose detail log may contain profile/server metadata."; show_tail_safe "$WATCHDOG_DETAIL_LOG" "watchdog detail log"; else echo "-- watchdog detail log skipped --"; echo "  Use: vless-go-doctor --verbose"; fi
section "Summary"; printf 'OK=%s WARN=%s FAIL=%s\n' "$OK_COUNT" "$WARN_COUNT" "$FAIL_COUNT"; [ "$FAIL_COUNT" -gt 0 ] && exit 2; [ "$WARN_COUNT" -gt 0 ] && exit 1; exit 0
