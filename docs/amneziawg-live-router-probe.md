...
## Результат первого живого роутера

Первый MIPSLE-тест выполнен 12 августа 2026 года на роутере с
`MemTotal: 124528 kB`, без swap и примерно 25-27 МБ доступной памяти.
Пятисекундный запуск использовал 5412 КБ RSS/HWM и 12 потоков; после cleanup
оставалось примерно 25 МБ доступной памяти. Маршруты, rules, управляемое
состояние Xray, `Proxy0` и активный слот не изменились.

Результат относится к проверенному роутеру. Он не заменяет отдельный тест на
устройстве, имеющем только 40 МБ физической RAM.
