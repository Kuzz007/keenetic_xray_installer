...
Установщик сначала проверяет весь bundle и раскладывает payload во временную
директорию на `/opt`, затем заменяет project-managed файлы через atomic rename.
При ошибке уже выполненные замены откатываются в обратном порядке.
