...
input_value = os.environ["VLESS_URL"].strip()
...
def _first_vless_from_text(text):
...
def resolve_vless_link(value):
    ...
    if parsed.scheme in ("http", "https"):
        try:
            req = urllib.request.Request(value, headers={"User-Agent": "xray-vless-failover/1.0"})
            with urllib.request.urlopen(req, timeout=20) as response:
                raw = response.read()
        except Exception as exc:
            raise SystemExit(f"ERROR: cannot download subscription: {exc}")

        plain = _decode_text(raw)
        link = _first_vless_from_text(plain)
        if link:
            return link, "subscription-plain"

        decoded = _try_base64_decode(plain)
        link = _first_vless_from_text(decoded)
        if link:
            return link, "subscription-base64"

        raise SystemExit("ERROR: subscription does not contain any vless:// link")

    raise SystemExit("ERROR: only vless:// links or http(s) subscription links are supported")


url, source_type = resolve_vless_link(input_value)
u = urllib.parse.urlparse(url)

if u.scheme != "vless":
    raise SystemExit("ERROR: resolved link is not vless://")
...
run_diagnostics() {
    ...
    echo "Статус Xray:"
    if [ -x "$XRAY_INIT" ]; then
        "$XRAY_INIT" status || true
    else
        echo "Init-скрипт Xray не найден: $XRAY_INIT"
    fi
    echo

    echo "Статус failover:"
    if [ -x "$FAILOVER_INIT" ]; then
        "$FAILOVER_INIT" status || true
    else
        echo "Init-скрипт failover не найден: $FAILOVER_INIT"
    fi
    echo
...
while true; do
    read_tty "Новая VLESS-ссылка или ссылка подписки Основного профиля: "
    PRIMARY_VLESS="$REPLY"

    if [ -z "$PRIMARY_VLESS" ]; then
        echo "ОШИБКА: ссылка Основного профиля пустая. Введите vless://... или http(s)-ссылку подписки."
        continue
    fi
    ...
    then
        echo "Ссылка Основного профиля принята."
        break
    fi

    echo
    echo "ОШИБКА: ссылка Основного профиля неверная или подписка недоступна."
    echo "Поддерживаются прямые ссылки vless://... и http(s)-ссылки подписок, внутри которых есть vless://..."
    echo "Попробуйте ввести ссылку заново."
    echo
    rm -f "$TMP_PRIMARY_CONFIG"
done

while true; do
    echo "VLESS-ссылка или ссылка подписки Резервного профиля опциональна. Для пропуска нажмите Enter."
    read_tty "Новая VLESS-ссылка или ссылка подписки Резервного профиля, опционально: "
    BACKUP_VLESS="$REPLY"

    if [ -z "$BACKUP_VLESS" ]; then
        echo "Резервный профиль пропущен."
        rm -f "$TMP_BACKUP_CONFIG"
        break
    fi
    ...
    then
        echo "Ссылка Резервного профиля принята."
        break
    fi

    echo
    echo "ОШИБКА: ссылка Резервного профиля неверная или подписка недоступна."
    echo "Введите корректную ссылку или нажмите Enter, чтобы пропустить Резервный профиль."
    echo
    rm -f "$TMP_BACKUP_CONFIG"
done
...
if [ "$REUSE_FAILOVER" = "1" ] && [ -s "$PRIMARY_STORE" ]; then
    echo "Используем сохранённую VLESS-ссылку Основного профиля."
    PRIMARY_VLESS="$(cat "$PRIMARY_STORE")"

    if [ -s "$BACKUP_STORE" ]; then
        echo "Используем сохранённую VLESS-ссылку Резервного профиля."
    ...
    read_tty "VLESS-ссылка или ссылка подписки Основного профиля: "
    PRIMARY_VLESS="$REPLY"

    echo "VLESS-ссылка или ссылка подписки Резервного профиля опциональна. Для пропуска нажмите Enter."
    read_tty "VLESS-ссылка или ссылка подписки Резервного профиля, опционально: "
    BACKUP_VLESS="$REPLY"
fi
...
while true; do
    if [ -z "$PRIMARY_VLESS" ]; then
        echo "ОШИБКА: ссылка Основного профиля пустая."
    else
    ...
    echo "Поддерживаются прямые ссылки vless://... и http(s)-ссылки подписок, внутри которых есть vless://..."
    read_tty "Введите VLESS-ссылку или ссылку подписки Основного профиля заново: "
    PRIMARY_VLESS="$REPLY"
    rm -f "$TMP_PRIMARY_CONFIG"
done
