...
SUBSCRIPTION_CRON_FILE="/opt/var/spool/cron/crontabs/root"
SUBSCRIPTION_UPDATE_LOG="/opt/var/log/xray-vless-subscription-update.log"
...
TMP_DIR="/opt/tmp"
CLEANUP_MAX_TMP_AGE_HOURS="6"
CLEANUP_KEEP_INSTALLER="1"
TEMP_HOST="127.0.0.1"
...
cleanup_temp_files() {
    REASON="${1:-ручная очистка}"

    mkdir -p "$TMP_DIR" /opt/var/log 2>/dev/null || true

    echo "Очистка временных файлов: $REASON"
    ...
    rm -f "$TMP_DIR"/xray-config-before-switch.json 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-config-before-vless-update.json 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-update-primary.json 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-update-backup.json 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-install-primary.json 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-install-backup.json 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-resolved-*.url 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-subscription-*.txt 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-subscription-*.decoded 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-core-*.zip 2>/dev/null || true
    rm -rf "$TMP_DIR"/xray-core-update-* 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-core-asset-list.txt 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-core-latest.json 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-core-prerelease.json 2>/dev/null || true
    rm -f "$TMP_DIR"/xray-version-check.json 2>/dev/null || true
    rm -f "$TMP_DIR"/*.fixed 2>/dev/null || true

    if [ "${CLEANUP_KEEP_INSTALLER:-1}" != "1" ]; then
        rm -f "$TMP_DIR"/xray_vless_failover.sh 2>/dev/null || true
        rm -f "$TMP_DIR"/xray-vless-failover-install.sh 2>/dev/null || true
    fi

    # Удаляем старые временные файлы этого проекта. BusyBox find обычно поддерживает -mtime, но не всегда -mmin.
    find "$TMP_DIR" -type f -name 'xray-*' -mtime +1 -exec rm -f {} \; 2>/dev/null || true
    find "$TMP_DIR" -type f -name 'vless-*' -mtime +1 -exec rm -f {} \; 2>/dev/null || true
    find "$TMP_DIR" -type d -name 'xray-*' -mtime +1 -exec rm -rf {} \; 2>/dev/null || true
}
...
show_storage_usage() {
    ...
    echo
    echo "Размер временной папки /opt/tmp:"
    du -sh "$TMP_DIR" 2>/dev/null || true

    echo
    echo "Крупные временные файлы проекта:"
    ls -lh "$TMP_DIR"/xray-* "$TMP_DIR"/vless-* 2>/dev/null | tail -n 20 || true
}
...
ensure_packages() {
    ...
    if ! command -v crond >/dev/null 2>&1 && [ ! -x /opt/sbin/crond ]; then
        NEED_UPDATE="1"
    fi
    ...
    if ! command -v unzip >/dev/null 2>&1; then
        echo "Устанавливаем unzip для обновления ядра Xray..."
        opkg install unzip || true
    fi

    if ! command -v crond >/dev/null 2>&1 && [ ! -x /opt/sbin/crond ]; then
        echo "Пробуем установить cron для автообновления подписок..."
        opkg install cron || opkg install cronie || true
    fi
...
profile_label = sys.argv[3]
previous_value = sys.argv[4].strip() if len(sys.argv) > 4 else ""
...
def parse_link_info(link):
    try:
        u = urllib.parse.urlparse(link)
        return {
            "fragment": urllib.parse.unquote(u.fragment or "").strip(),
            "host": u.hostname or "",
            "port": str(u.port or 443),
            "username": urllib.parse.unquote(u.username or ""),
        }
    except Exception:
        return {"fragment": "", "host": "", "port": "", "username": ""}

if mode == "refresh" and previous_value:
    prev = parse_link_info(previous_value)

    if prev["fragment"]:
        for link in links:
            cur = parse_link_info(link)
            if cur["fragment"] == prev["fragment"]:
                print(link)
                raise SystemExit(0)
    ...
    if prev["host"]:
        for link in links:
            cur = parse_link_info(link)
            if cur["host"] == prev["host"] and cur["port"] == prev["port"]:
                print(link)
                raise SystemExit(0)
...
subscription_update_now() {
    [ -x "$SUBSCRIPTION_UPDATE_CMD" ] && "$SUBSCRIPTION_UPDATE_CMD" || echo "Команда обновления подписок не найдена: $SUBSCRIPTION_UPDATE_CMD"
    pause_menu
}

configure_subscription_auto_update() {
    echo "Автообновление подписок"
    echo
    echo "1 Включить / изменить интервал"
    echo "2 Отключить"
    echo "3 Показать текущие cron-задания"
    echo "0 Назад"
    printf "Ваш выбор: "; IFS= read -r CH

    case "$CH" in
        1)
            printf "Интервал в часах, например 6, 12 или 24: "
            IFS= read -r HOURS
            case "$HOURS" in
                ''|*[!0-9]*)
                    echo "Нужно указать число часов."
                    pause_menu
                    return
                    ;;
            esac
            [ "$HOURS" -ge 1 ] 2>/dev/null || { echo "Интервал должен быть >= 1."; pause_menu; return; }

            mkdir -p /opt/var/spool/cron/crontabs /opt/var/log

            CRON_TMP="/opt/tmp/root.cron.$$"
            if [ -f "$SUBSCRIPTION_CRON_FILE" ]; then
                grep -v 'xray-vless-subscription-update' "$SUBSCRIPTION_CRON_FILE" > "$CRON_TMP" 2>/dev/null || true
            else
                : > "$CRON_TMP"
            fi

            echo "0 */$HOURS * * * $SUBSCRIPTION_UPDATE_CMD --auto >> $SUBSCRIPTION_UPDATE_LOG 2>&1 # xray-vless-subscription-update" >> "$CRON_TMP"
            mv "$CRON_TMP" "$SUBSCRIPTION_CRON_FILE"
            chmod 600 "$SUBSCRIPTION_CRON_FILE"

            if [ -x /opt/etc/init.d/S10cron ]; then
                /opt/etc/init.d/S10cron restart || true
            elif command -v crond >/dev/null 2>&1; then
                crond -c /opt/var/spool/cron/crontabs -L /opt/var/log/cron.log 2>/dev/null || true
            elif [ -x /opt/sbin/crond ]; then
                /opt/sbin/crond -c /opt/var/spool/cron/crontabs -L /opt/var/log/cron.log 2>/dev/null || true
            else
                echo "ПРЕДУПРЕЖДЕНИЕ: crond не найден. Установите cron через opkg или запустите cron вручную."
            fi

            echo "Автообновление подписок включено: каждые $HOURS ч."
            ;;
        2)
            if [ -f "$SUBSCRIPTION_CRON_FILE" ]; then
                CRON_TMP="/opt/tmp/root.cron.$$"
            ...
            [ -x /opt/etc/init.d/S10cron ] && /opt/etc/init.d/S10cron restart || true
            echo "Автообновление подписок отключено."
            ;;
        3)
            echo "Файл cron: $SUBSCRIPTION_CRON_FILE"
            echo
            grep 'xray-vless-subscription-update' "$SUBSCRIPTION_CRON_FILE" 2>/dev/null || echo "Заданий автообновления нет."
            ;;
        0)
            return
            ;;
...
uninstall_failover() {
    ...
    [ -x "$FAILOVER_INIT" ] && "$FAILOVER_INIT" stop >/dev/null 2>&1 || true
    rm -f /opt/bin/failover /opt/bin/vless-failover-update /opt/bin/xray-failover-update /opt/bin/xray-core-update /opt/bin/vless-subscription-update
...
while true; do
    ...
    echo "7 Автообновление подписок"
...
echo "12 Удаление / деинсталляция"
    echo "0 Выход"
    echo
...
validate_profile_input() {
    ...
    PROFILE_SOURCE_RESULT="$SOURCE_VALUE"
...
ask_required_profile_input() {
    ...
    while true; do
        ...
        if validate_profile_input "$PROFILE" "$CANDIDATE" "$HOST" "$PORT" "$OUT_CONFIG"; then
            ...
            PROFILE_SOURCE_INPUT_RESULT="$CANDIDATE"
            return 0
        fi
...
create_subscription_update_command() {
    echo "Создаём команду обновления подписок..."
...
TMP_DIR="/opt/tmp"
SOCKS_PORT="10808"
LOGFILE="/opt/var/log/xray-vless-subscription-update.log"
...
resolve_refresh_profile() {
    ...
    if [ ! -s "$SOURCE_FILE" ] && [ -s "$STORE_FILE" ]; then
        cp "$STORE_FILE" "$SOURCE_FILE"
        chmod 600 "$SOURCE_FILE" 2>/dev/null || true
    fi

    [ -s "$SOURCE_FILE" ] || {
        log_msg "Источник профиля $LABEL не задан. Пропускаем."
        return 1
    }

    SOURCE_VALUE="$(cat "$SOURCE_FILE")"
    OLD_VALUE="$(cat "$STORE_FILE" 2>/dev/null || true)"

    if [ -x "$RESOLVER" ]; then
        if ! NEW_VALUE="$("$RESOLVER" refresh "$SOURCE_VALUE" "$LABEL" "$OLD_VALUE")"; then
            log_msg "ОШИБКА: не удалось обновить профиль $LABEL из источника."
            return 1
        fi
    else
...
apply_active_config() {
    ...
    case "$ACTIVE" in
        backup)
            ...
            ACTIVE_VALUE="$BACKUP_NEW"
            ;;
        *)
            ACTIVE_PROFILE="primary"
            ACTIVE_VALUE="$PRIMARY_NEW"
            ;;
    esac

    if [ -z "$ACTIVE_VALUE" ]; then
        log_msg "Активный профиль $ACTIVE_PROFILE пустой. Используем Основной."
        ACTIVE_PROFILE="primary"
        ACTIVE_VALUE="$PRIMARY_NEW"
    fi

    TMP_CONFIG="$TMP_DIR/xray-subscription-active-$ACTIVE_PROFILE.json"
    PROFILE_NAME="$ACTIVE_PROFILE" \
    VLESS_URL="$ACTIVE_VALUE" \
...
XRAY_BIN="$(get_xray_bin)"
[ -n "$XRAY_BIN" ] || { log_msg "ОШИБКА: xray не найден."; exit 1; }
[ -x "$GENERATOR" ] || { log_msg "ОШИБКА: генератор не найден: $GENERATOR"; exit 1; }
[ -x "$RESOLVER" ] || { log_msg "ОШИБКА: resolver не найден: $RESOLVER"; exit 1; }
[ -s "$ROUTER_IP_STORE" ] || { log_msg "ОШИБКА: LAN-IP не найден: $ROUTER_IP_STORE"; exit 1; }

log_msg "Начинаем обновление подписок."

PRIMARY_NEW=""
BACKUP_NEW=""

resolve_refresh_profile "primary" "$PRIMARY_SOURCE" "$PRIMARY_STORE" PRIMARY_NEW || {
    log_msg "ОШИБКА: Основной профиль не обновлён. Прерываем."
    exit 1
}

if [ -s "$BACKUP_SOURCE" ] || [ -s "$BACKUP_STORE" ]; then
    resolve_refresh_profile "backup" "$BACKUP_SOURCE" "$BACKUP_STORE" BACKUP_NEW || {
        log_msg "ПРЕДУПРЕЖДЕНИЕ: Резервный профиль не обновлён."
        BACKUP_NEW="$(cat "$BACKUP_STORE" 2>/dev/null || true)"
    }
fi

TMP_PRIMARY_CONFIG="$TMP_DIR/xray-subscription-primary.json"
ROUTER_LAN_IP="$(cat "$ROUTER_IP_STORE")"
PROFILE_NAME="primary" \
VLESS_URL="$PRIMARY_NEW" \
LISTEN_HOST="$ROUTER_LAN_IP" \
LISTEN_PORT="$SOCKS_PORT" \
...
if [ -n "$BACKUP_NEW" ]; then
    TMP_BACKUP_CONFIG="$TMP_DIR/xray-subscription-backup.json"
    PROFILE_NAME="backup" \
    VLESS_URL="$BACKUP_NEW" \
...
printf "%s\n" "$PRIMARY_NEW" > "$PRIMARY_STORE"
chmod 600 "$PRIMARY_STORE"

if [ -n "$BACKUP_NEW" ]; then
    printf "%s\n" "$BACKUP_NEW" > "$BACKUP_STORE"
    chmod 600 "$BACKUP_STORE"
else
    rm -f "$BACKUP_STORE"
fi

if ! apply_active_config; then
    log_msg "ОШИБКА: не удалось применить обновлённый config. Откат."
    [ -s "$OLD_CONFIG" ] && cp "$OLD_CONFIG" "$XRAY_CONFIG"
    [ -x "$FAILOVER_INIT" ] && "$FAILOVER_INIT" start >/dev/null 2>&1 || true
    exit 1
fi
...
if ! "$INIT_SCRIPT" restart; then
    log_msg "ОШИБКА: Xray не перезапустился. Откат."
    [ -s "$OLD_CONFIG" ] && cp "$OLD_CONFIG" "$XRAY_CONFIG"
    "$INIT_SCRIPT" restart >/dev/null 2>&1 || true
    [ -x "$FAILOVER_INIT" ] && "$FAILOVER_INIT" start >/dev/null 2>&1 || true
    exit 1
...
log_msg "Обновление подписок завершено успешно."
...
cleanup_temp_files "перед установкой"
...
if [ "$REUSE_FAILOVER" = "1" ] && { [ -s "$PRIMARY_SOURCE" ] || [ -s "$PRIMARY_STORE" ]; }; then
    echo "Используем сохранённый источник Основного профиля."
    if [ -s "$PRIMARY_SOURCE" ]; then
        PRIMARY_SOURCE_VALUE="$(cat "$PRIMARY_SOURCE")"
    else
        PRIMARY_SOURCE_VALUE="$(cat "$PRIMARY_STORE")"
    fi
    PRIMARY_VLESS="$PRIMARY_SOURCE_VALUE"

    if ! validate_profile_input "primary" "$PRIMARY_SOURCE_VALUE" "$ROUTER_LAN_IP" "$SOCKS_PORT" "$TMP_PRIMARY_CONFIG"; then
        echo "Сохранённая ссылка Основного профиля неверная или подписка недоступна. Нужно ввести заново."
        ask_required_profile_input "primary" "$ROUTER_LAN_IP" "$SOCKS_PORT" "$TMP_PRIMARY_CONFIG"
        PRIMARY_VLESS="$PROFILE_INPUT_RESULT"
        PRIMARY_SOURCE_VALUE="$PROFILE_SOURCE_INPUT_RESULT"
    else
        PRIMARY_VLESS="$PROFILE_RESOLVED_RESULT"
    fi

    if [ -s "$BACKUP_SOURCE" ] || [ -s "$BACKUP_STORE" ]; then
        echo "Используем сохранённый источник Резервного профиля."
        if [ -s "$BACKUP_SOURCE" ]; then
            BACKUP_SOURCE_VALUE="$(cat "$BACKUP_SOURCE")"
        else
            BACKUP_SOURCE_VALUE="$(cat "$BACKUP_STORE")"
        fi
        BACKUP_VLESS="$BACKUP_SOURCE_VALUE"

        if ! validate_profile_input "backup" "$BACKUP_SOURCE_VALUE" "127.0.0.1" "$TEMP_BACKUP_PORT" "$TMP_BACKUP_CONFIG"; then
            ...
            BACKUP_VLESS="$PROFILE_RESOLVED_RESULT"
        fi
    else
...
cleanup_temp_files "после установки"
show_storage_usage
...
echo "vless-subscription-update"
