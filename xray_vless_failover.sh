...
while true; do
    ...
    echo "6 История переключений"
    echo "7 Ручное переключение профиля"
    echo "8 Проверить версию Xray"
    echo "9 Удаление / деинсталляция"
    echo "0 Выход"
    echo
    ...
    case "$MENU_CHOICE" in
        ...
        2) show_realtime_log ;;
        3) run_diagnostics ;;
        4) [ -x "$XRAY_CORE_UPDATE_CMD" ] && "$XRAY_CORE_UPDATE_CMD" || echo "Команда обновления ядра не найдена"; pause_menu ;;
        5) settings_menu ;;
        6) show_history ;;
        7) manual_switch ;;
        8) check_xray_version ;;
        9) uninstall_failover ;;
        0) echo "Выход."; exit 0 ;;
