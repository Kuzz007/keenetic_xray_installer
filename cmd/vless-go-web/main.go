...
var actions = []action{
	{ID: "status", Label: "Обновить статус", Description: "Показать статус failover", Command: []string{"/opt/bin/vless-go-failover", "status"}, Timeout: 10 * time.Second, Group: "overview", Tone: "secondary"},
	{ID: "watchdog-status", Label: "Статус watchdog", Description: "Показать статус daemon watchdog", Command: []string{"/opt/bin/vless-go-watchdog", "status"}, Timeout: 10 * time.Second, Group: "overview", Tone: "secondary"},
	{ID: "doctor", Label: "Запустить doctor", Description: "Полная диагностика установки", Command: []string{"/opt/bin/vless-go-doctor"}, Timeout: 90 * time.Second, Group: "diagnostics", Tone: "primary"},
	{ID: "switch-primary", Label: "Переключить на основной", Description: "Активировать primary source", Command: []string{"/opt/bin/vless-go-failover", "switch", "primary"}, Timeout: 60 * time.Second, Group: "failover", Tone: "purple"},
	{ID: "switch-backup", Label: "Переключить на резервный", Description: "Активировать backup source", Command: []string{"/opt/bin/vless-go-failover", "switch", "backup"}, Timeout: 60 * time.Second, Group: "failover", Tone: "amber"},
	{ID: "update-all", Label: "Обновить активный конфиг", Description: "Перегенерировать активный Xray config", Command: []string{"/opt/bin/vless-go-failover", "update-active"}, Timeout: 90 * time.Second, Group: "sources", Tone: "cyan"},
	{ID: "auto-update-run", Label: "Запустить автообновление", Description: "Выполнить selector-aware auto-update сейчас", Command: []string{"/opt/bin/vless-go-auto-update", "run"}, Timeout: 120 * time.Second, Group: "updates", Tone: "cyan"},
	{ID: "restart-xray", Label: "Перезапустить Xray", Description: "Перезапустить S24xray", Command: []string{"/opt/etc/init.d/S24xray", "restart"}, Timeout: 30 * time.Second, Group: "maintenance", Tone: "secondary"},
  ...
  <div class="grid">
    <div>
      <section class="panel"><div class="section-kicker">FAILOVER</div><h2>Управление переключением</h2><div class="action-grid">
        {{range actionsIn .Actions "failover"}}<form method="post" action="/action" class="{{.Tone}}"><input type="hidden" name="id" value="{{.ID}}"><input type="hidden" name="token" value="{{$.Token}}"><button type="submit">{{.Label}}</button><div class="desc">{{.Description}}</div></form>{{end}}
        {{range actionsIn .Actions "overview"}}<form method="post" action="/action" class="{{.Tone}}"><input type="hidden" name="id" value="{{.ID}}"><input type="hidden" name="token" value="{{$.Token}}"><button type="submit">{{.Label}}</button><div class="desc">{{.Description}}</div></form>{{end}}
      </div></section>

      <section class="panel"><div class="section-kicker">ИСТОЧНИКИ</div><h2>Источники</h2><form method="post" action="/set-source"><input type="hidden" name="token" value="{{.Token}}"><div class="form-grid"><div><label>Слот</label><select name="slot"><option value="primary">primary / основной</option><option value="backup">backup / резервный</option></select></div><div><label>Выбор профиля</label><input name="selector" placeholder="first или index:7" value="first"></div></div><label>VLESS-ссылка или URL подписки</label><input name="source" type="password" autocomplete="off" placeholder="vless://... или https://..."><button type="submit">Сохранить источник</button></form><p class="footer-note">Приватные значения источников принимаются формой, но не выводятся обратно на страницу.</p></section>

      <section class="panel"><div class="section-kicker">ВЫБОР ПРОФИЛЕЙ</div><h2>Выбор профилей</h2><form method="post" action="/set-selector"><input type="hidden" name="token" value="{{.Token}}"><div class="split"><div><label>Основной профиль</label><input name="primary" value="{{.PrimarySelector}}"></div><div><label>Резервный профиль</label><input name="backup" value="{{.BackupSelector}}"></div></div><button type="submit">Сохранить selectors</button></form></section>
