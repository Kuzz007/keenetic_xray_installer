...
func (s *Server) telegramLoop() {
	...
	if err := s.setBotCommands(); err != nil { log.Printf("telegram setMyCommands: %v", err) }
	for { updates, err := s.getUpdates(); if err != nil { log.Printf("telegram: %v", err); time.Sleep(5*time.Second); continue }; for _, u := range updates { s.handleTelegramUpdate(u) } }
}
...
func (s *Server) routerList() string { s.mu.Lock(); defer s.mu.Unlock(); ids:=make([]string,0,len(s.cfg.Routers)); for id:=range s.cfg.Routers { ids=append(ids,id) }; sort.Strings(ids); online:=0; items:=[]string{}; for _,id:=range ids { if rt:=s.cfg.Routers[id]; rt!=nil && time.Since(rt.LastSeen)<30*time.Second { online++ } }; items=append(items,prettyRouterListHeader(len(ids),online)); for _,id:=range ids { rt:=s.cfg.Routers[id]; if rt==nil { continue }; name:=id; if strings.TrimSpace(rt.Name)!="" { name=rt.Name }; items=append(items,prettyRouterListItem(name,rt.LastSeen,rt.Status)) }; return strings.Join(items,"\n\n") }
...
func (s *Server) sendMessageWithKeyboard(chatID int64, text string, keyboard inlineKeyboard) { if s.cfg.BotToken=="" { return }; payload:=map[string]any{"chat_id":chatID,"text":limit(text,3900)}; if len(keyboard.InlineKeyboard)>0 { payload["reply_markup"]=keyboard }; body,_:=json.Marshal(payload); url:=fmt.Sprintf("https://api.telegram.org/bot%s/sendMessage",s.cfg.BotToken); resp,err:=http.Post(url,"application/json",bytes.NewReader(body)); if err!=nil { log.Printf("sendMessage: %v",err); return }; _,_=io.Copy(io.Discard,resp.Body); _=resp.Body.Close() }
