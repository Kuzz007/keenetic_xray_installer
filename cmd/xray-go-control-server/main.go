...
type Config struct { ConfigPath string; Listen string; BotToken string; AdminUserID int64; Routers map[string]*Router; CertFile string; KeyFile string; CertFingerprint string; StateFile string; AllowLegacyHTTP bool }
type Router struct { ID string; Name string; Token string; LastSeen time.Time; LastTransport string; Status string; RequestedAgentChannel string; Queue []Command; Results []Result }
...
func main() {
	...
	mux.HandleFunc("/agent/bootstrap", s.handleAgentBootstrap)
	mux.HandleFunc("/health", handleHealth)
	mux.HandleFunc("/version", handleVersion)
	mux.HandleFunc("/fingerprint", s.handleFingerprint)
	go s.telegramLoop()
	log.Printf("xray-go-control-server listen=%s routers=%d tls-sha256-fingerprint=%s legacy-http=%t %s", cfg.Listen, len(cfg.Routers), fingerprint, cfg.AllowLegacyHTTP, versionLine())
	log.Fatal(serveControlServer(cfg, mux))
}
...
func (s *Server) handlePoll(w http.ResponseWriter, r *http.Request) {
	...
	s.mu.Lock(); rt.LastSeen = time.Now(); rt.LastTransport = requestTransport(r); if hb.Status != "" { rt.Status = hb.Status }
...
func (s *Server) telegramLoop() {
	...
	if err := s.setBotCommands(); err != nil { log.Printf("telegram setMyCommands: %s", redactTelegramError(err,s.cfg.BotToken)) }
	for { updates, err := s.getUpdates(); if err != nil { log.Printf("telegram: %s", redactTelegramError(err,s.cfg.BotToken)); time.Sleep(5*time.Second); continue }; for _, u := range updates { s.handleTelegramUpdate(u) } }
}
...
func (s *Server) handleTelegramUpdate(u tgUpdate) {
	...
	case text == "/routers": s.sendMessageWithKeyboard(chatID, s.routerList(), s.routersKeyboard())
...
func (s *Server) routersKeyboard() inlineKeyboard {
	s.mu.Lock(); ids := make([]string,0,len(s.cfg.Routers)); for id := range s.cfg.Routers { ids = append(ids,id) }; sort.Strings(ids)
	rows := [][]inlineButton{}; for _, id := range ids { rt := s.cfg.Routers[id]; text := id; if rt != nil && strings.TrimSpace(rt.Name) != "" { text = rt.Name }; if rt != nil { dot,_ := onlineState(rt.LastSeen); text = dot+" "+text }; rows = append(rows, []inlineButton{{Text:text, CallbackData:"router:"+id}}) }; s.mu.Unlock()
	rows = append(rows, []inlineButton{{Text:"🔄 Обновить", CallbackData:"routers"},{Text:"⬅️ Назад", CallbackData:"menu"}}); return inlineKeyboard{InlineKeyboard: rows}
}
...
func (s *Server) handleActionCallback(chatID int64, messageID int, data string) {
	parts := strings.SplitN(data, ":", 3); if len(parts) != 3 { s.editOrSendMessageWithKeyboard(chatID,messageID,"Некорректная кнопка",mainMenuKeyboard()); return }
	name, routerID := parts[1], parts[2]; if messageID > 0 { s.setActiveMenu(routerID, chatID, messageID) }
	if name == "results" { s.editOrSendMessageWithKeyboard(chatID,messageID,s.results(routerID),s.currentRouterKeyboard(routerID)); return }
	action := callbackAction(name); if action == "" { s.editOrSendMessageWithKeyboard(chatID,messageID,"Действие не поддерживается: "+name,routerKeyboard(routerID)); return }
	id, err := s.enqueue(routerID, Command{Action: action}); if err != nil { s.editOrSendMessageWithKeyboard(chatID,messageID,err.Error(),s.routersKeyboard()); return }
	s.editOrSendMessageWithKeyboard(chatID,messageID,s.routerMenuTextWithExtra(routerID,"⏳ Команда в очереди\n"+id),s.currentRouterKeyboard(routerID))
}
