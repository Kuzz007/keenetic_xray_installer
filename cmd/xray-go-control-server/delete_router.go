...
func (s *Server) sendDeleteRouterConfirm(chatID int64, routerID string) {
	...
	name := routerID
	if rt != nil { name = rt.Name }
	s.mu.Unlock()
	if rt == nil {
		s.sendMessageWithKeyboard(chatID, "Роутер не найден: "+routerID, routersKeyboard(s.routerIDs()))
		return
	}
	s.sendMessageWithKeyboard(chatID, "Удалить роутер из control-server?\n\n"+name+" ("+routerID+")\n\nАгент на самом роутере не удаляется автоматически.", deleteRouterConfirmKeyboard(routerID))
}
...
func (s *Server) deleteRouter(chatID int64, routerID string) {
	...
	if err != nil { s.cfg.Routers[routerID] = rt }
	s.mu.Unlock()
	if err != nil {
