...
func deleteRouterConfirmKeyboard(routerID string) inlineKeyboard {
	return inlineKeyboard{InlineKeyboard: [][]inlineButton{
		...
		{{Text: "↩️ Отмена", CallbackData: "router:" + routerID}, {Text: "📡 К списку", CallbackData: "routers"}},
	}}
}
...
func (s *Server) sendDeleteRouterConfirm(chatID int64, routerID string) {
	...
	s.sendMessageWithKeyboard(chatID,
		"🗑 Удаление роутера\n\n📡 "+name+"\nID: "+routerID+"\n\nРоутер будет удалён только из registry control-server.\nАгент на самом роутере не удаляется автоматически.",
...
func (s *Server) deleteRouter(chatID int64, routerID string) {
	...
	if rt == nil {
		s.mu.Unlock()
		s.sendMessageWithKeyboard(chatID, "ℹ️ Роутер уже отсутствует: "+routerID, s.routersKeyboard())
		return
	}

	if len(s.cfg.Routers) <= 1 {
		s.mu.Unlock()
		s.sendMessageWithKeyboard(chatID, "⚠️ Нельзя удалить последний роутер из registry.", routerKeyboard(routerID))
		return
	}
	...
	if err != nil {
		s.sendMessageWithKeyboard(chatID, "❌ Роутер не удалён:\n"+err.Error(), routerKeyboard(routerID))
		return
	}

	s.sendMessageWithKeyboard(chatID, "✅ Роутер удалён из registry:\n"+routerID, s.routersKeyboard())
}
