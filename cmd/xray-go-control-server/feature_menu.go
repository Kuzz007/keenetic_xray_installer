...
func routerKeyboardForStatus(routerID, status string) inlineKeyboard {
	...
	// XMUX/XUDP is shown only for agents that explicitly advertise mux_config.
	// Old legacy agents keep the normal full menu but do not receive a button
	// for commands they cannot safely execute.
	if f["mux_config"] {
		routeRow = append(routeRow, inlineButton{Text: "⚙️ XMUX/XUDP", CallbackData: "mux:" + routerID})
	}
