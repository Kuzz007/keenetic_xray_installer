...
<section class="cards"><div class="card" style="--accent:var(--green)"><div class="label">Активный слот</div><div class="value">{{.Active}}</div></div><div class="card" style="--accent:var(--cyan)"><div class="label">Primary</div><div class="value">{{.Primary}}</div></div><div class="card" style="--accent:var(--purple)"><div class="label">Backup</div><div class="value">{{.Backup}}</div></div><div class="card" style="--accent:var(--amber)"><div class="label">Watchdog</div><div class="value">{{.Watchdog}}</div></div><div class="card" style="--accent:var(--blue)"><div class="label">Recovery</div><div class="value">{{.Recovery}}</div></div><div class="card" style="--accent:var(--cyan)"><div class="label">Proxy0</div><div class="value">{{.Proxy0}}</div></div><div class="card" style="--accent:var(--red)"><div class="label">SOCKS auth</div><div class="value">{{.SocksAuth}}</div></div></section>
<div class="grid"><div>
...
<section class="panel"><div class="kicker">Proxy0 / SOCKS</div><h2>Логин и пароль SOCKS5</h2><form method="post" action="/socks-auth"><input type="hidden" name="token" value="{{.Token}}"><input type="hidden" name="action" value="enable"><div class="formgrid"><div><label>Логин</label><input name="user" autocomplete="username" placeholder="xray"></div><div><label>Пароль</label><input type="password" name="pass" autocomplete="new-password" placeholder="сложный пароль"></div></div><label class="inline"><input type="checkbox" name="skip_proxy0" value="1"> Не применять Proxy0 автоматически, только включить auth в Xray</label><button>Включить SOCKS auth</button></form><p class="note">Разрешены символы: A-Z a-z 0-9 . _ @ : -. Пароль не выводится обратно. При включении активный Xray config будет перегенерирован.</p><form method="post" action="/socks-auth"><input type="hidden" name="token" value="{{.Token}}"><input type="hidden" name="action" value="disable"><button>Отключить SOCKS auth</button></form></section>
...
func main() {
	...
	mux.HandleFunc("/socks-auth", post(token, listen, handleSocksAuth))
...
func quickStatus() string {
	...
	for _, cmd := range fallback {
		parts = append(parts, "$ "+strings.Join(cmd, " "), runCommand(cmd, 10*time.Second))
	}
...
func recoverySummary(s string) string {
	...
	if strings.Contains(lower, "hourly recovery: enabled") {
		return "enabled"
	}
	if strings.Contains(lower, "hourly recovery: disabled") {
		return "disabled"
	}
	if strings.Contains(lower, "health: ok") {
		return "health OK"
	}
	return "unknown"
}
...
func proxy0Summary(s string) string {
	lower := strings.ToLower(s)
	if strings.Contains(lower, "proxy iface: proxy0") || strings.Contains(lower, "proxy0") {
		return "Proxy0"
	}
	return "unknown"
}

func socksAuthSummary() string {
	out := runCommand([]string{"/opt/bin/vless-go-socks-auth", "status"}, 10*time.Second)
	lower := strings.ToLower(out)
	if strings.Contains(lower, "socks auth: enabled") {
		return "enabled"
	}
	if strings.Contains(lower, "socks auth: disabled") {
		return "disabled"
	}
	return "unknown"
}
...
func handleSocksAuth(r *http.Request) string {
	...
	case "enable":
		...
		if user == "" || pass == "" {
			return "ОШИБКА: логин и пароль обязательны"
		}
		cmd := []string{"/opt/bin/vless-go-socks-auth", "enable", user, pass}
		shown := "$ /opt/bin/vless-go-socks-auth enable " + user + " <hidden>"
		if r.FormValue("skip_proxy0") == "1" {
			cmd = append(cmd, "--skip-proxy0")
			shown += " --skip-proxy0"
		}
		return shown + "\n" + runCommand(cmd, 180*time.Second)
	case "disable":
		cmd := []string{"/opt/bin/vless-go-socks-auth", "disable"}
		return "$ " + strings.Join(cmd, " ") + "\n" + runCommand(cmd, 180*time.Second)
	default:
		return "ОШИБКА: неизвестное действие SOCKS auth"
	}
}
